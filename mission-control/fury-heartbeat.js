...
const args = JSON.stringify({ sessionKey: 'agent:customer-researcher:main' });
...
try {
  ...
  const fs = require('fs');
  const tempFile = require('path').join(__dirname, '_hb_temp.json');
  fs.writeFileSync(tempFile, args);
  try {
    const result2 = execSync(`npx convex run agents:heartbeat "$(cat _hb_temp.json)"`, {
      encoding: 'utf-8',
      cwd: __dirname,
      shell: 'bash'
    });
